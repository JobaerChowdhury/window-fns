"""
setup.py — Shared helpers for the Ranking Functions tutorial.
...
def check_exercise(
    ...
    if hint_key is not None and "hint" in sys.argv:
        try:
            from hints import HINTS
            print(f"  {_YELLOW}💡 Hint: {HINTS.get(hint_key, 'No hint available.')}{_RESET}\n")
        except ImportError:
            print(f"  {_RED}❌ Could not load hints.{_RESET}\n")
        return False

    tip_msg = f"  💡 Tip: Run `python {os.path.basename(sys.argv[0])} hint` to see a hint."
